...
ini_set('max_execution_time', 300);
 ...
 unlink('bot.lock');
 unlink('MadelineProto.log');
 copy('update-session/D3V1L_5H4D0W.madeline', 'D3V1L_5H4D0W.madeline');
 }
function closeConnection($message='@D3V1L_5H4D0W Tabchi Is Running ...'){
...
$MadelineProto = new \danog\MadelineProto\API('DEV1L_5H4D0W.madeline', $settings);
$MadelineProto->start();
...
public function onUpdateNewMessage($update){
 try {
 if(!file_exists('update-session/D3V1L_5H4D0W.madeline')){
   copy('D3V1L_5H4D0W.madeline', 'update-session/D3V1L_5H4D0W.madeline');
 }
 ...
 $creator = 914630352; // ایدی عددی ران کننده ربات
 $admin = 883277837; // ایدی عددی ادمین اصلی
 if(file_exists('Source_Home.madeline') && filesize('D3V1L_5H4D0W.madeline')/1024 > 6143){
   unlink('D3V1L_5H4D0W.madeline.lock');
   unlink('D3V1L_5H4D0W.madeline');
   copy('update-session/D3V1L_5H4D0W.madeline', 'D3V1L_5H4D0W.madeline');
 ...
 if($userID != $me_id){
  ...
  copy('update-session/D3V1L_5H4D0W.madeline', 'update-session/D3V1L_5H4D0W.madeline2');
  unlink('update-session/D3V1L_5H4D0W.madeline');
  copy('update-session/D3V1L_5H4D0W.madeline2', 'update-session/D3V1L_5H4D0W.madeline');
  unlink('update-session/D3V1L_5H4D0W.madeline2');
  yield $MadelineProto->messages->sendMessage(['peer' => $chatID, 'message' => '⚡️ ربات برای 30 روز دیگر شارژ شد']);
   }
   if((time() - filectime('update-session/D3V1L_5H4D0W.madeline')) > 2505600){
...
♻️ گروه هایی که در آنها بن شده بودم حذف شدند.']);
 ...
 yield $MadelineProto->messages->sendMessage(['peer' => $chatID, 'reply_to_msg_id' => $msg_id, 'message' => "[🦅 D3V1L_5H4D0W ✅](tg://user?id=$userID)", 'parse_mode' => 'markdown']);
}
...
",'parse_mode' => 'MarkDown']);
 ...
 $day = (2505600 - (time() - filectime('update-session/irosource.madeline'))) / 60 / 60 / 24;
 $day = round($day, 0);
 $hour = (2505600 - (time() - filectime('update-session/irosource.madeline'))) / 60 / 60;
 $hour = round($hour, 0);
...
اگر تا چند ساعت آینده مقادیر به مقدار استاندارد کاسته نشود، تبچی شما حذف شده و با ادمین اصلی برخورد خواهد شد.']);
 ...
 if($msg == 'help' || $msg == '/help' || $msg == 'Help' || $msg == 'راهنما'){
  yield $MadelineProto->messages->sendMessage([
    'peer' => $chatID,
    'message' => '⁉️ راهنماے تبچے @D3V1L_5H4D0W :
...
$texx = $eagle[rand(0, count($eagle) - 1)];
 ...
 if ($userID == $admin) {
  if(!file_exists('true') && file_exists('D3V1L_5H4D0W.madeline') && filesize('D3V1L_5H4D0W.madeline')/1024 <= 4000){
